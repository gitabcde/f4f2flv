...
char F4F_FLVHEADER[]={'F','L','V',0x01,0x05,0x00,0x00,0x00,0x09,0x00,0x00,0x00,0x00};
...
{
	char* n=(char*)userdata;
	char* test=(char*)buff;
	size_t datalen=size*count;
	char f4fname[200];
	memset(f4fname,0,200);
	sprintf(f4fname,"%s",n);
...
void CF4FPaser::SetF4m(char* f4murl)
{
	...
	while(f4m_str.find("Access Denied")!=std::string::npos)
	  {
	    f4m_str="";
	    WriteToStringFromUrl(f4murl,f4m_str);
	  }
...
void CF4FPaser::GetF4MInfo(F4MINFO* pF4mInfo)
{
	...
	while(tmp<count)
	{
		...
		if(f4m_str.find("url=",bootstrap_begin)==std::string::npos || f4m_str.find("url=",bootstrap_begin)>bootstrap_end)
		{
		  ...
		  int space_count=bootstrap_begin;
		  char str_space[2];
		  str_space[1]='\0';
		  while(space_count<bootstrap_end)
		    {
		      str_space[0]=(*(f4m_str.c_str()+space_count));
		      if(strstr(cb64,str_space)!=NULL)
			break;
		      space_count++;
		    }
		  bootstrap_begin=space_count;
		  space_count=bootstrap_end;
		  while(space_count>bootstrap_begin)
		    {
		      str_space[0]=(*(f4m_str.c_str()+space_count));
		      if(strstr(cb64,str_space)!=NULL)
			break;
...
void CF4FPaser::CreateFlvFile(char* flvname)
{
	...
	flvfile.write(F4F_FLVHEADER,sizeof(F4F_FLVHEADER));
	flvfile.close();
...
void CF4FPaser::WriteFlvDataFromF4file(char* f4file,char* flvname)
{
	...
	if(prev_f4filename!=f4file)
	{
		m_f4ftaginfo.largesize=false;
		m_f4ftaginfo.size.size_32=0;
		m_f4ftaginfo.size.size_64=0;
		memset(m_f4ftaginfo.name,0,4);
		m_f4ftaginfo.pos_beg=0;
		prev_f4filename=f4file;
	}

	while(GetTagInfoFromFile(f4file,"mdat")==0)
	{

		std::cout<<"begin loop in WriteFlvDataFromF4file"<<std::endl;
		char buffer[F4F_BUFFER_SIZE];
		std::cout<<"filename is "<<f4file<<std::endl;
		std::cout<<"tagname is "<<std::string(m_f4ftaginfo.name)<<", largesize is "<<(m_f4ftaginfo.largesize?"true":"false")<<" ,size is "<<m_f4ftaginfo.size.size_64<<std::endl;
		std::cout<<"current f4file is "<<f4file<<std::endl;
		std::cout<<"current f4file's pos_beg is "<<m_f4ftaginfo.pos_beg<<std::endl;
		std::cout<<"current f4ffile's data size is "<<m_f4ftaginfo.size.size_32<<std::endl;
		FixTimeStampAndWriteFlvData(flvname,f4file);
		m_f4ftaginfo.pos_beg+=m_f4ftaginfo.size.size_64;
...
int CF4FPaser::GetTagInfoFromFile(char* filepath,char* tagname)
{
	...
	fs.seekg(m_f4ftaginfo.pos_beg);
	uint64_t test=fs.tellg();
	if(test==filelength)
		return -1;
	std::cout<<"m_f4ftaginfo.pos_beg is "<<m_f4ftaginfo.pos_beg<<std::endl;
	while((m_f4ftaginfo.pos_beg+m_f4ftaginfo.size.size_64)<filelength)
	{
		m_f4ftaginfo.size.size_64=0;
		fs.read((char*)&m_f4ftaginfo.size.size_32,4);
		if(!isbigendian())
			cvtendian(&m_f4ftaginfo.size.size_32,4);
		if(m_f4ftaginfo.size.size_32==1)
		{
			m_f4ftaginfo.largesize=true;
			fs.seekg(m_f4ftaginfo.pos_beg+8);
			fs.read((char*)&m_f4ftaginfo.size.size_64,8);
			if(!isbigendian())
				cvtendian(&m_f4ftaginfo.size.size_64,8);
		}
		else
		{
			m_f4ftaginfo.largesize=false;
		...
		fs.seekg(m_f4ftaginfo.pos_beg+4);
		fs.read((char*)m_f4ftaginfo.name,4);
		std::cout<<"tagname is "<<m_f4ftaginfo.name<<std::endl;
		if((*((uint32_t*)m_f4ftaginfo.name))==(*((uint32_t*)tagname)))
		{
			if(m_f4ftaginfo.pos_beg+m_f4ftaginfo.size.size_64!=filelength)
			{
				std::cout<<"the mdat's size is wrong"<<std::endl;
				m_f4ftaginfo.size.size_64=filelength-m_f4ftaginfo.pos_beg;
			}
			fs.close();
			return 0;
		}
		fs.seekg(m_f4ftaginfo.pos_beg+m_f4ftaginfo.size.size_64);
		m_f4ftaginfo.pos_beg+=m_f4ftaginfo.size.size_64;
...
void CF4FPaser::FixTimeStampAndWriteFlvData(char* flvname,char* f4fname)
{
	...
	if(m_f4ftaginfo.largesize)
		now_pos=m_f4ftaginfo.pos_beg+16;
	else
		now_pos=m_f4ftaginfo.pos_beg+8;
...
//myf4f.SetF4m("http://vodhdkure-vh.akamaihd.net/z/f/2014/02/22/1113184/146979-trt-dizi-osmanli-tokadi-30_,270,360,480,p.mp4.csmil/manifest.f4m?hdnea=st=1393288846~exp=1393290246~acl=/*~hmac=a4c449775be3797b0d00a828247c7cbcf873da7bbdd6dc86f89f9c3c0978a69c&g=IVZQXAKMQJEF&hdcore=3.2.0&plugin=jwplayer-3.2.0.1");
myf4f.SetF4m("http://svtplay2h-f.akamaihd.net/z/world/open/20140214/1355920-006A/TRADGARDSKAMPEN-006A-113dd985e93e3c00_,900,348,564,1680,2800,.mp4.csmil/manifest.f4m?g=NDOKTEQMSJFT&hdcore=3.2.0");
F4MINFO myf4minfo;
...
curl_easy_setopt(hd_curl,CURLOPT_USERAGENT,"mozilla/5.0 (compatible; msie 9.0; windows nt 6.1; wow64; trident/5.0)");
//      curl_easy_setopt(hd_curl,CURLOPT_COOKIE,"_alid_=zxdDjQtA1QUFMiqFk1OCjw==; hdntl=exp=1393375431~acl=%2f*~data=hdntl~hmac=caf2013f5bb0da8b5c4d74c7624e77ba99191549cca91454d9551601b99e82cc");
curl_easy_setopt(hd_curl,CURLOPT_URL,mytmpstr.c_str());
curl_easy_setopt(hd_curl,CURLOPT_WRITEFUNCTION,&WriteToFile);
curl_easy_setopt(hd_curl,CURLOPT_WRITEDATA,"tmpf4f");
CURLcode ret_code=curl_easy_perform(hd_curl);
if(ret_code!=CURLE_OK)
{
...
std::cout<<"cannot download video"<<std::endl;
 ...
 myf4f.WriteFlvDataFromF4file("tmpf4f","videourl.flv");
prev_seg=myf4minfo.last_seg[0];
prev_frag=myf4minfo.last_seg_fragcount[0];
}
curl_easy_cleanup(hd_curl);
